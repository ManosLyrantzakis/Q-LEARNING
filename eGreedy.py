...
# -- Νευρωνικό Δίκτυο (Q-Network) --
class DQN(nn.Module):
    def __init__(self):
        super(DQN, self).__init__()
        self.fc1 = nn.Linear(4, 64)  # 4 χαρακτηριστικά (y, pipe_x, pipe_height, bird_velocity)
        self.fc2 = nn.Linear(64, 64)
        self.fc3 = nn.Linear(64, 2)  # 2 δράσεις: 0 (Do nothing), 1 (Jump)
...
# -- Μνήμη Εμπειριών για Replay Learning --
class ReplayMemory:
...
# -- AI Εκπαίδευση με Deep Q-Learning --
...
def select_action(state, epsilon):
    if random.random() < epsilon:
        return random.randint(0, 1)  # Τυχαία δράση (εξερεύνηση)
    else:
        with torch.no_grad():
            return policy_net(torch.tensor(state, dtype=torch.float32).to(device)).argmax().item()
